(() => {
  ...
  async function reloadCloud(){
    ...
    const [{data:tasks,error:tasksErr},{data:requests,error:reqErr},{data:templates,error:tplErr}]=await Promise.all([
    ...
    if(tasksErr||reqErr||tplErr){
      console.error("reloadCloud error",{tasksErr,reqErr,tplErr});
      toast(`Ошибка загрузки: ${(tasksErr||reqErr||tplErr).message}`);
    }
